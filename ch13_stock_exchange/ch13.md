...
#### Matching Engine 的 I/O 全貌

把上述 Deep Dive 的邊界重組對應到實際的資料流，Matching Engine **同時是 mmap Event Store 的 consumer 與 producer**，並且自己擁有一個本地 ring buffer 作為輸出緩衝：

| 方向               | 資料                                     | 經過的機制                                            |
| :----------------- | :--------------------------------------- | :---------------------------------------------------- |
| **讀入**     | `NewOrderEvent`                        | mmap Event Store（pull，consumer offset）             |
| **內部狀態** | Order Book、OrderMap、嵌入式 Order State | 純 process memory（HashMap + DoublyLinkedList）       |
| **產出**     | `OrderFilledEvent`                     | 先進**自己的 ring buffer** → Sequencer → mmap |

**輸出端為何要先過自己的 ring buffer？**

Matching Engine **不會直接寫 mmap**——這是「單一 Writer」原則的硬性要求（詳見後續 Step 3「為什麼每個 Event Store 只能有一個 Sequencer」）。撮合產出 `OrderFilledEvent` 後，流程是：

```
Matching Engine 撮合完
    ↓
寫入自己的 ring buffer（Single Producer，無鎖）
    ↓
Sequencer 從這個 ring buffer pull
    ↓
注入 sequence ID
    ↓
寫入 mmap Event Store
    ↓
（Market Data Publisher / Reporter / Warm Instance 各自從 Event Store 拉走）
```

這個設計同時滿足三個約束：

1. **Latency Determinism**：Matching Engine 寫 ring buffer 是 single-producer，無 lock contention
2. **全域順序**：所有事件最終經由 Sequencer 注入單調遞增 sequence ID
3. **多消費者解耦**：mmap Event Store 是 Pub-Sub 匯流排，每個下游元件用自己的 offset 獨立消費

ring buffer 與 mmap 在 Matching Engine 上**並存且串聯**——這也是後續「為什麼 mmap 與 Ring Buffer 必須並存？」一節的最具體案例。
...
#### 程式實作走讀：`internal/marketdata/publisher.go`

##### 為什麼 K 線特別適合 Ring Buffer

K 線的資料形態有三個特性，使其幾乎是 Ring Buffer 的教科書範例：

1. **大小可預期**：每個（symbol × interval）只保留「最近 N 根」（書中是 100 根），新進來就把最舊的擠掉。
2. **寫入頻率固定**：每個 interval 結束才 Push 一次，不會爆量。
3. **讀取永遠是「最近 N 根」**：前端 K 線圖、L1 即時報價只看尾端，不查 3 小時前那根 1 分鐘 K。

若改用 Go slice：每次 `append` 可能觸發 grow + 複製，舊資料變垃圾要 GC。對追求 **Latency Determinism** 的撮合系統而言，這種「偶爾打嗝幾毫秒」的 GC pause 不可接受。Ring Buffer 用一塊固定的記憶體不停輪轉，**生命週期內不分配、不釋放、不 GC**。

##### 資料結構：固定陣列 + 兩個整數

```go
type RingBuffer struct {
    data  [ringBufferCapacity]*domain.Candlestick  // 固定 100 槽
    head  int  // 下一個要寫入的位置
    count int  // 目前已寫入數量（最多 = capacity）
}
```

**注意是 `[100]*Candlestick` 而不是 `[]*Candlestick`**——這個差別很關鍵：

- 陣列大小編譯期決定，整個 `RingBuffer` 物件**內聯**儲存（不像 slice 還要間接指 backing array）
- 沒有 `len/cap`、沒有自動擴容
- 對應前述「**固定大小**：預先分配記憶體，無動態 `new` / `delete`」

兩個整數的職責分工：

- `head`：**下一個要寫入的格子位置**（不是最新的那筆所在位置）
- `count`：目前實際裝了幾筆，buffer 還沒滿之前用來算正確的 start
...
1. **無條件覆蓋**：`data[head] = c`——舊資料的指針就被丟了，不需要 free。
2. **head 前進並 wrap**：`(head+1) % cap`，到底了自動繞回 0。
3. **count 飽和於 cap**：滿了之後就停止增加。

**圖解 Push 5 筆到容量為 4 的 buffer**（為畫圖把容量縮到 4）：
...
Push(A) → head=1, count=1
data: [A  _  _  _]
          ↑head

Push(B) → head=2, count=2
data: [A  B  _  _]
             ↑head

Push(C) → head=3, count=3
data: [A  B  C  _]
                ↑head

Push(D) → head=0 (wrap!), count=4 (滿)
data: [A  B  C  D]
       ↑head

Push(E) → head=1, count=4 (停在 cap)
data: [E  B  C  D]   ← A 被覆蓋
          ↑head
```

最舊的一筆永遠在 `head` 指的位置（buffer 滿時）；最新的一筆永遠在 `head-1`。
...
**(1) `start` 的計算**：要從「head 往回退 n 格」當成起點。`(head - n + cap) % cap` 的 `+ cap` 是處理 Go 負數模運算的習慣做法（Go 的 `%` 對負數會回負，加 cap 後再 mod 確保是正的）。

**(2) 為什麼不能直接 `data[start:start+n]`**：因為 ring buffer 可能跨越邊界。延續上面的例子，buffer 滿後 `head=1`，要拿最近 3 筆：

- `start = (1 - 3 + 4) % 4 = 2`
- 應拿索引 2, 3, 0 → 即 [C, D, E]
- 直接 slice `data[2:5]` 會 out of bounds

所以必須用 for loop 配 `(start+i) % cap` 一個一個複製。

##### Ring Buffer 不是孤立存在：兩段式 K 線生命週期

書中「兩段式記憶體策略」在程式中體現為**兩個 map**：

```go
type Publisher struct {
    candles map[string]*RingBuffer    // 已完成的 K 線（每個 symbol 一個 ring buffer）
    states  map[string]*candleState   // 正在建構中的當前 K 線
    ...
}
```
...
```
┌─────────────────────────────────────────────────────────────┐
│  階段 1：building（在 states map 中）                          │
│  → 每筆成交來都更新 OHLCV，同一根 K 線一直被改                  │
│  → 不進 RingBuffer，避免 1 分鐘內反覆 Push 造成尾端跳動         │
├─────────────────────────────────────────────────────────────┤
│  階段 2：completed（被 rotateCandlesticks Push 進 RingBuffer）  │
│  → 1 分鐘 ticker 觸發，把 building → ring buffer，清空 state   │
│  → 從此這根 K 線是 immutable，等著被覆蓋或讀取                  │
└─────────────────────────────────────────────────────────────┘
```

**building 階段（`updateCandle`）**：

```go
if !state.hasData {
    // 第一筆成交：建立新 K 線，OHLC 都設為當下價格
    state.current = &domain.Candlestick{
        Open: exec.Price, High: exec.Price, Low: exec.Price, Close: exec.Price,
        Volume: exec.Quantity, ...
    }
} else {
    // 後續成交：更新 High/Low、改寫 Close、累加 Volume
    if exec.Price > c.High { c.High = exec.Price }
    if exec.Price < c.Low  { c.Low  = exec.Price }
    c.Close = exec.Price
    c.Volume += exec.Quantity
}
```

這就是 OHLCV 的標準算法：第一筆設 Open，每筆比 High/Low，最後一筆是 Close，Volume 累加。

**rotation 階段（`rotateCandlesticks`）**：每分鐘把 `state.current` Push 進 ring buffer，重設 `state.hasData = false`。下一分鐘第一筆成交進來，又從新的 Candlestick 開始。

**讀取時合併兩段（`GetCandles`）**：

```go
result = rb.GetRecent(count)              // 已完成的（從 ring buffer）
if state.hasData {
    result = append(result, state.current) // 還在建構的（從 states map）
}
```

這樣前端拿到的 K 線陣列才會包含「正在跳動的當前那根」，否則永遠延遲 1 分鐘才看得到最新成交。

##### 目前實作的妥協 vs 書中的 Lock-free + Padding

前述 Ring Buffer 三點優化中，**程式為了教學簡化只做了第一點（固定大小），後兩點刻意保留作為下一階段優化**：

**(1) `sync.RWMutex` 不是 Lock-free**

實作用 `sync.RWMutex` 保護所有讀寫（`processExecutionEvent`、`rotateCandlesticks`、`GetCandles` 都加鎖）。書中要的是：

- **Single Producer**：只有 application loop thread 會 Push，根本不需要鎖
- **Multiple Consumer**：讀者用 atomic load `head/count`，靠記憶體屏障保證可見性

要改成 Lock-free 的雛形大概是：

```go
type RingBuffer struct {
    data [ringBufferCapacity]*domain.Candlestick
    head atomic.Uint64  // 用 atomic 取代 int + mutex
}

func (rb *RingBuffer) Push(c *domain.Candlestick) {
    h := rb.head.Load()
    rb.data[h % ringBufferCapacity] = c
    rb.head.Store(h + 1)  // store with release semantics
}
```

注意 head 設成單調遞增（不取模、不繞回），讀的時候才 mod。這樣 reader 可以分辨「我上次讀到 seq=42、現在 head=50，所以追上 8 筆」，不會跟舊資料的覆蓋混淆——這是 **LMAX Disruptor** 的核心 trick。

**(2) 沒有 Cache Line Padding（False Sharing 問題）**

`head` 和 `count` 兩個 int 大概率被編譯器塞在同一個 64-byte cache line。如果未來改 Multi-Producer，兩個 thread 即使各自更新 head 跟 count，也會因為共用 cache line 而互相 invalidate，效能崩潰——這就是 **False Sharing**。

C++ 用 `alignas(64)`，Go 沒有對應語法，常見做法是手動塞 padding：

```go
type RingBuffer struct {
    data [ringBufferCapacity]*domain.Candlestick
    head atomic.Uint64
    _    [56]byte  // padding：64 - 8 = 56 bytes，把 head 撐滿一個 cache line
    tail atomic.Uint64
    _    [56]byte
}
```
...
| 書中描述                         | 程式對應                                  | 完成度    |
| :------------------------------- | :---------------------------------------- | :-------- |
| 固定大小、預先分配               | `[ringBufferCapacity]*Candlestick` 陣列 | ✅        |
| 無 `new` / `delete`，避免 GC | Push 直接覆蓋指針                         | ✅        |
| 兩段式（記憶體 + 磁碟）          | 記憶體段做完，磁碟 sink 未實作            | ⚠️ 一半 |
| Lock-free                        | `sync.RWMutex`                          | ❌ 為簡化 |
| Cache line padding               | 無                                        | ❌ 為簡化 |
| OHLCV 累積                       | `updateCandle` 完整實作                 | ✅        |
| 跨 interval rotation             | `rotateCandlesticks` + 1 分鐘 ticker    | ✅        |

教學上這個實作的價值是先看懂 **Ring Buffer 的本質——head 指針 + 模運算 + 兩段式 K 線生命週期**。Lock-free 與 Padding 是 production 的下一步優化（LMAX Disruptor 風格），等需要時再加進來，不會推翻現在的結構。
...
### 為什麼 mmap 與 Ring Buffer 必須並存？

讀到這裡會出現一個合理的疑問：mmap 是共享記憶體、ring buffer 也是固定大小的記憶體緩衝，**為什麼系統要同時使用兩者？** 答案是它們位於**完全不同的層級**，各自解決不同的問題，缺一不可。

#### 三個出現位置的對照

ch13.md 通篇提到的 ring buffer 與 mmap，其實散布在系統的三個不同層級：

| #  | 位置                                | 機制                                    | 作用域                         |
| :- | :---------------------------------- | :-------------------------------------- | :----------------------------- |
| ① | 各 Component 寫入 Sequencer 前      | **Ring Buffer**（多個，各一份）   | In-process，每個 producer 一份 |
| ② | Sequencer → 所有消費者的中央匯流排 | **mmap Event Store**              | Inter-process，全域唯一        |
| ③ | Market Data Publisher 內部 K 線快取 | **Ring Buffer**（每 symbol 一份） | In-process，純記憶體           |

對應 ch13.md 前述的 Sequencer 流程：

> 1. 各 Component 將事件先寫入自己本地的 **Ring Buffer**
> 2. Sequencer 從這些 Ring Buffer 拉取（pull）事件
> 3. 注入 Sequence ID 後寫入 Event Store（mmap）

所以 ring buffer 與 mmap 是**串聯**的，不是平行替代。

#### 為什麼不能只用 mmap、不用 ring buffer？

**(1) 寫入端的 Lock Contention 問題**

mmap Event Store 的核心約束是 **單一 Writer**（前述「強制規定：單一 Writer（Sequencer）是 Event Store 不可妥協的原則」）。但系統有**多個來源**要產生事件：Gateway、Matching Engine、Reporter。如果它們直接寫 mmap，會發生：

```
[Gateway]    [Matching]   [Reporter]
    ↓             ↓            ↓
    └─────────────┼────────────┘
                  ↓
            爭搶 mmap 寫入權
            （Lock Contention，違反 Latency Determinism）
```

正確設計是讓每個 component 先把事件丟到**自己的 ring buffer**（單 Producer 單 Consumer，無爭奪），Sequencer 用 pull 模式逐一從各 ring buffer 拉取、注入 sequence ID、串成全域順序：

```
[Gateway]→ringbuf →┐
[Matching]→ringbuf →├─ Sequencer (single writer) → mmap Event Store
[Reporter]→ringbuf →┘
```

ring buffer 在這裡的角色是「**上游解耦緩衝**」：吸收 component 的瞬間突發，平滑成 Sequencer 可以穩定消費的流。

**(2) 跨 Process 通訊只能靠 mmap**

即使單機架構，Order Manager、Matching Engine、Market Data Publisher、Reporter **各自是獨立 Process**（為了 CPU Pinning、隔離 GC、隔離崩潰）。

| 機制                                            | 跨 Process？                                     |
| :---------------------------------------------- | :----------------------------------------------- |
| Go channel / 一般 ring buffer（process 記憶體） | ❌ 完全不行                                      |
| mmap（共享記憶體）                              | ✅ 多 process 把同一檔案映射進各自 address space |

ring buffer 是純 in-process 的資料結構——**它只活在一個 process 的 heap 裡**。Process A 的 ring buffer，Process B 看不到。要讓多個 process 看到同一份事件流，**只有 mmap 可選**（網路 +500μs，違反延遲預算）。

> 註：本實驗為簡化，把所有元件放在同一個 Go process 裡，所以用 channel 取代 ring buffer + mmap 的組合。書中的生產級架構是多 process 的。

#### 為什麼不能只用 ring buffer、不用 mmap？

**(1) Ring Buffer 是「會丟資料的」**

Ring buffer 的本質是**滿了就覆蓋舊資料**。這對「上游緩衝」與「最近 100 根 K 線快取」是 OK 的，因為這些資料是衍生品或可丟失的緩衝。

但 Event Store 是**事件溯源的黃金真相**：事件日誌（Event Log）是永久不可變的。如果 Event Store 用 ring buffer：

- 系統重啟後，舊事件被覆蓋掉 → 無法 replay → Warm Instance 起不來
- Reporter 慢了 → 事件被覆蓋 → 報表永久缺資料
- 監管要查 1 個月前的單 → 沒了 → **直接違規**

所以 Event Store **必須是 append-only、永不覆蓋**，這個語義 ring buffer 給不了。

**(2) Multi-Consumer 各自 offset 的需求**

Event Store 是 Pub-Sub 匯流排：每個元件各自維護一個讀取指針（offset），**以自己的速度消費**。

| 消費者                | 消費速度                |
| :-------------------- | :---------------------- |
| Matching Engine       | 微秒級（撮合熱路徑）    |
| Market Data Publisher | 微秒級（廣播）          |
| Reporter              | 毫秒級（寫 DB，慢很多） |
| Warm Instance         | 同 Hot 速度（同步追上） |

ring buffer 的「滿了覆蓋」在這個場景**致命**——只要最慢的 Reporter 一卡住，ring buffer 就會把它還沒讀的事件覆蓋掉。mmap 配合**順序遞增的 sequence ID + 持久化檔案**，可以讓每個消費者用自己的 offset 獨立讀取：

- 快的消費者：offset 已經到 seq=10000
- 慢的消費者：offset 還在 seq=5000
- 兩者**互不干擾**，事件永遠在那裡等慢的追上

這是 Kafka 的核心模型，mmap 在本機重現了它。

**(3) 故障復原（Crash Recovery）**

ring buffer 在 process 記憶體裡 → process 一掛，資料全沒。
mmap 的 backing file 在 `/dev/shm`（甚至可同步到一般磁碟）→ process 掛掉，資料還在檔案裡，重啟後重新映射就能繼續。

Hot-Warm Failover 就是依賴這個性質：Hot 掛了，Warm 接手後**重新 mmap 同一個 Event Store 檔案**，sequence ID 接著往下遞增，零資料遺失。

#### 在 MDP 的 ring buffer 又是另一回事

第 ③ 個位置（MDP 內部的 K 線 ring buffer）跟前兩個沒有任何關係——它解決的是**完全不同的問題**：

> 「最近 100 根 K 線」需要被前端高速查詢，但**不需要**被 replay 或 audit。

K 線是「衍生資料」（從 Event Store 的 OrderFilledEvent 即時聚合而來）。它的特性是：

| 屬性                   | K 線 ring buffer                  | mmap Event Store         |
| :--------------------- | :-------------------------------- | :----------------------- |
| 是否可重建？           | ✅ 從 Event Store replay 可重新算 | 不可重建（自身就是源頭） |
| 是否需 cross-process？ | ❌ MDP 自己用                     | ✅ 多個 process 共用     |
| 是否需保留全部歷史？   | ❌ 最近 100 根足夠                | ✅ 全部                  |
| 寫入頻率               | 每分鐘一次                        | 每筆事件一次             |

所以 K 線**用 ring buffer 是對的**——固定容量、無 GC、單一 MDP process 內部用。如果硬把 K 線塞進 mmap Event Store，會：

- 浪費磁碟空間（K 線可重算）
- 增加 Event Store 寫入速率（影響關鍵路徑延遲）
- 增加無謂的 audit log 體積
...
│  Ring Buffer 適用的地方                                            │
│  ─────────────────────                                             │
│  ✅ 上游緩衝（吸收突發、解耦多 producer）                           │
│  ✅ 衍生資料快取（K 線、L1 報價，可重算的）                          │
│  ✅ 單 process / 單執行緒內部                                       │
│  ✅ 容忍丟資料（覆蓋舊資料是 feature 不是 bug）                     │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│  mmap Event Store 適用的地方                                       │
│  ─────────────────────────                                         │
│  ✅ 跨 process 通訊（共享記憶體唯一選項）                           │
│  ✅ 黃金真相（永不覆蓋、可 replay、可 audit）                        │
│  ✅ Multi-Consumer 各自 offset（Pub-Sub 匯流排）                    │
│  ✅ Crash Recovery（backing file 持久化）                           │
└─────────────────────────────────────────────────────────────────┘
```

**一句話總結**：ring buffer 是 **本地、會丟、極快的緩衝層**，mmap 是 **全域、不丟、跨 process 的中樞神經**。兩者協作的順序是：

> Component → 本地 ring buffer（解耦）→ Sequencer → mmap Event Store（持久共享）→ 各消費者 → （MDP 內部再用一個 ring buffer 快取衍生品）
